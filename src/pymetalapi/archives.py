...
import re
from pymetalapi.util import get_random_user_agent, get_element
...
class MetalArchives:
    ...
    def get_band_data(url):
        result = {}
        response = MetalArchives.session.get(url, headers={'User-Agent': get_random_user_agent()})
        tree = html.fromstring(response.content)
        result["name"] = get_element(tree=tree, sel_type='text', sel='h1.band_name > a:nth-child(1)')
        result["url"] = get_element(tree=tree, sel_type='href', sel='h1.band_name > a:nth-child(1)')
        result["genre"] = \
            get_element(tree=tree, sel_type='text', sel='dl.float_right > dd:nth-child(2)')
        result["themes"] = \
            get_element(tree=tree, sel_type='text', sel='dl.float_right > dd:nth-child(4)')
        result["label"] = \
            get_element(tree=tree, sel_type='text',
                        sel='dl.float_right > dd:nth-child(6) > a:nth-child(1)')
        result["country"] = \
            get_element(tree=tree, sel_type='text',
                        sel='dl.float_left > dd:nth-child(2) > a:nth-child(1)')
        result["location"] = \
            get_element(tree=tree, sel_type='text', sel='dl.float_left > dd:nth-child(4)')
        result["status"] = \
            get_element(tree=tree, sel_type='text', sel='dl.float_left > dd:nth-child(6)')
        result["formed"] = \
            get_element(tree=tree, sel_type='text', sel='dl.float_left > dd:nth-child(8)')
        result["years"] = \
            get_element(tree=tree, sel_type='text', sel='dl.clear > dd:nth-child(2)')
